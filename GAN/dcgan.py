...
class Generator(BasedGenerator):
    ...
    def _build(self):
        self.fc1 = tf.keras.layers.Dense(4*4*256, activation=None, kernel_regularizer=self.l2_regularizer)
        self.bn1 = tf.keras.layers.BatchNormalization()
        self.relu1 = tf.keras.layers.ReLU()
        self.reshape = tf.keras.layers.Reshape((4, 4, 256))
        self.deconv1 = tf.keras.layers.Conv2DTranspose(256, kernel_size=(5,5), strides=(3,3), padding='same', kernel_regularizer=self.l2_regularizer)
        self.bn2 = tf.keras.layers.BatchNormalization()
        self.relu2 = tf.keras.layers.ReLU()
        self.deconv2 = tf.keras.layers.Conv2DTranspose(64, kernel_size=(3,3), strides=(2,2), padding='same', kernel_regularizer=self.l2_regularizer)
        self.bn3 = tf.keras.layers.BatchNormalization()
        self.relu3 = tf.keras.layers.ReLU()
        self.deconv3 = tf.keras.layers.Conv2DTranspose(self.channel, kernel_size=(5,5), strides=(1,1), activation=tf.nn.tanh, padding='valid', kernel_regularizer=self.l2_regularizer)
    ...
    def __call__(self, outputs, trainable=True):
        ...
        outputs = self.relu1(outputs, training=trainable)
        ...
        outputs = self.relu2(outputs, training=trainable)
        outputs = self.deconv2(outputs, training=trainable)
        outputs = self.bn3(outputs, training=trainable)
        outputs = self.relu3(outputs, training=trainable)
...
class Discriminator(BasedDiscriminator):
    ...
    def _build(self):
        self.conv1 = tf.keras.layers.Conv2D(128, kernel_size=(3, 3), padding='same', activation=None, kernel_regularizer=self.l2_regularizer)
        self.bn1 = tf.keras.layers.BatchNormalization()
        self.leaky_relu1 = tf.keras.layers.LeakyReLU()
        self.conv2 = tf.keras.layers.Conv2D(128, kernel_size=(4, 4), strides=(2,2), padding='same', activation=None, kernel_regularizer=self.l2_regularizer)
        self.bn2 = tf.keras.layers.BatchNormalization()
        self.leaky_relu2 = tf.keras.layers.LeakyReLU()
        self.conv3 = tf.keras.layers.Conv2D(128, kernel_size=(4, 4), strides=(2,2), padding='same', activation=None, kernel_regularizer=self.l2_regularizer)
        self.bn3 = tf.keras.layers.BatchNormalization()
    ...
    def __call__(self, outputs, trainable=True):
        ...
        outputs = self.conv3(outputs, training=trainable)
        outputs = self.bn3(outputs, training=trainable)
