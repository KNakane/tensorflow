...
class GAN(BasedGAN):
    ...
    def build(self):
        ...
        self.discriminator_model = [['fc', 512, tf.nn.leaky_relu],
                                    ['BN'],
    ...
    def inference(self, inputs, batch_size):
        self.z = tf.random.uniform((batch_size, self._z_dim), dtype=tf.float32)
        #self.z = tf.random_normal((batch_size, self._z_dim), dtype=tf.float32)
        fake_img = self.G(self.z)

        fake_logit = tf.nn.sigmoid(self.D(fake_img))
        real_logit = tf.nn.sigmoid(self.D(inputs, reuse=True))
        return real_logit, fake_logit, fake_img

    def loss(self, real_logit, fake_logit):
        d_loss = - (tf.reduce_mean(tf.log(real_logit + self.eps)) + tf.reduce_mean(tf.log(1 - fake_logit + self.eps)))
        g_loss = - tf.reduce_mean(tf.log(fake_logit + self.eps))
        return d_loss, g_loss
    
    def optimize(self, d_loss, g_loss):
        global_step = tf.train.get_or_create_global_step()
        opt_D = tf.train.AdamOptimizer(2e-4, beta1=0.5).minimize(d_loss, global_step, var_list=self.D.var)
