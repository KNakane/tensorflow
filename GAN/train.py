...
"""
    ...
    data = Load(FLAGS.data)
    ## setting models
    model = eval(FLAGS.network)(z_dim=FLAGS.z_dim, size=data.size, channel=data.channel, opt=FLAGS.opt, lr=FLAGS.lr, conditional=FLAGS.conditional, class_num=data.output_dim, l2_reg=FLAGS.l2_norm, trainable=True)

    #training
    trainer = GANTrain(FLAGS, message, data, model, FLAGS.network)
    trainer.train()
"""
