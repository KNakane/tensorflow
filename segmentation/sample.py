...
def train(parser):
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=model_unet.teacher,
    ...
    m_iou,conf_mat = tf.metrics.mean_iou(model_unet.teacher, model_unet.outputs, ld.DataSet.length_category())
    ...
    for epoch in range(epochs):
        ...
        # Evaluation
        if epoch % 1 == 0:
            loss_train,accuracy_train, miou_train = sess.run([cross_entropy,accuracy,m_iou], feed_dict=train_dict)
            #loss_test = sess.run(cross_entropy, feed_dict=test_dict)
            loss_test, accuracy_test, miou_test = sess.run([cross_entropy,accuracy,m_iou], feed_dict=test_dict)
            #accuracy_test = sess.run(accuracy, feed_dict=test_dict)
            print("Epoch:{0}  [Train] Loss:{1}  Accuracy:{2:%.3f}  Mean IOU:")
            print("Epoch:", epoch, \
                  "[Train] Loss:", loss_train, " Accuracy:", accuracy_train, "Mean IOU:",miou_train, \
                  "[Test]  Loss:", loss_test, "Accuracy:", accuracy_test, "Mean IOU:",miou_test)
            #print("[Train] Loss:", loss_train, " Accuracy:", accuracy_train, "Mean IOU:",miou_train)
            #print("[Test]  Loss:", loss_test, "Accuracy:", accuracy_test, "Mean IOU:",miou_test)
    ...
    loss_test,accuracy_test = sess.run([cross_entropy, accuracy], feed_dict=test_dict)
    #accuracy_test = sess.run(accuracy, feed_dict=test_dict)
    print("Result")
